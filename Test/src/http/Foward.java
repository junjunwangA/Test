...
public class Foward {
    ...
    public static void main(String[] args) throws Exception {
    	...
    	InetSocketAddress inet = new InetSocketAddress(10002);
    	
        HttpServer server = HttpServer.create(inet, 0);
//        server.createContext("/forward", new MyHandler());
        server.createContext(null, new MyHandler());
        
        server.setExecutor(null); // creates a default executor
       
        server.start();
        System.out.println( server.getAddress()+"  "+ InetAddress.getLocalHost() );
        System.out.println("forward server started");
    }
